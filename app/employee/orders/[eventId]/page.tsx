...
export default function OrderQueuePage() {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        ...
        // Buscar os pedidos
        const ordersResponse = await fetch(`https://localhost:7027/api/Orders/event/${eventId}`);
        ...
        // Adicionar campo status (temporário até ser implementado no backend)
        const processedOrders = ordersData.map((order: any) => ({
          ...order,
          status: order.status || "pending" // Se o backend já tiver esse campo, usará ele
        }));
  ...
  const markOrderAsCompleted = async (orderId: number) => {
    try {
      ...
      const response = await fetch(`https://localhost:7027/api/Orders/${orderId}/complete`, {
        method: 'PUT',
        headers: {
